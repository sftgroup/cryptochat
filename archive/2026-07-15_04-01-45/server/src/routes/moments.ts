...
/** GET /api/moments — List moments (friends + public) */
momentsRouter.get('/', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    // Show own + friends' (any visibility) + public moments from non-friends
    const moments = await prisma.moment.findMany({
      where: {
        OR: [
          { userId: { in: [req.user!.userId, ...friendIds] } },  // own + friends (all visibility)
...
momentsRouter.delete('/:id', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    // Cascade delete likes + comments (Prisma schema has onDelete: Cascade)
