...
> 版本: v2.3.0 | 更新: 2026-07-15 | Ceres深度集成完成
...
> 注：S2.6 轮询已被 WebSocket 实时推送替代（比 PRD 设计的 2s 轮询更优）
...
| S1.4 Ceres DID 铸造 | 首次使用通过 CeresInviteCore.bindInviterBySig 铸造 DID，建立链上身份 | ✅ 已实现（CeresMintPage + App.tsx 自动跳转） |
| S1.5 Ceres DID 验证 | 启动时调用 Ceres API `/v1/batch-check` 验证 DID 状态、获取邀请图谱 | ✅ 已实现（App.tsx + registry.ts） |
| S1.6 ECDH 密钥生成与管理 | 浏览器本地生成 ECDH-P256 密钥对，公钥关联到 Ceres DID | ✅ 已实现（铸造时 updateProfile + getUrls） |
| S1.7 个人资料 | 查看/编辑 displayName、bio、avatarUrl | ✅ 已实现 |

### 2.2 单聊 (DM)

| 场景 | 描述 | 状态 |
|------|------|------|
| S2.1 添加好友 | 通过钱包地址或 Ceres 邀请码搜索，发送好友请求（双方均需 Ceres DID） | ✅ 已实现（前后端双重校验 Ceres DID） |
...
| S2.8 转账/红包 | 聊天中发送红包（TransferCard + TransferForm 组件） | ✅ 已实现 |
| S2.9 文件传输 | 聊天中通过 IPFS 上传/分享文件（FileCard 支持图片/视频/下载） | ✅ 已实现 |
| S2.10 表情选择器 | EmojiPicker 弹窗选择表情插入输入框 | ✅ 已实现 |
| S2.11 消息状态 | 已发送/已读状态（✓ 已发送 / ✓✓ 已读） | ✅ 已实现 |
| S2.12 离线消息 | 离线期间的消息在登录后同步（/api/dm/inbox） | ✅ 已实现 |
...
| S3.11 退出群聊 | 成员自行退群 | ✅ 已实现 |
| S3.12 移除成员 | 群主踢人 | ✅ 已实现 |
| S3.13 修改群信息 | 群主修改群名/描述 | ✅ 已实现 |
| S3.14 群聊 @提及 | @某个群成员发送定向通知（后端 mentions + WebSocket push + 前端选择器） | ✅ 已实现 |
...
| S4.4 点赞/评论 | 对动态点赞或评论 | ✅ 已实现 |
| S4.5 可见性控制 | 仅好友可见 / 公开 | ✅ 已实现（Prisma + 后端 + 前端选择器） |
| S4.6 动态删除 | 作者删除自己的动态（DELETE /api/moments/:id + 前端按钮） | ✅ 已实现 |
...
| S5.2 **Ceres 邀请图谱** | 可视化展示我的 Ceres 邀请树（inviter → me → invitees → descendants） | ✅ 已实现（CeresInviteTree 组件，三级关系树） |
| S5.3 社交发现 | 基于 Ceres 二级、三级关系推荐可能认识的人 | ✅ 已实现（SocialDiscovery 组件，双策略发现） |
