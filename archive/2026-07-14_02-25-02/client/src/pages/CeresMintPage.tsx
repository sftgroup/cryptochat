 ...
 *
 * Ceres v2.3 — DID 铸造走 OxaChain L1 主网 (Chain ID 19505)
 *
 * Contracts (OxaChain L1):
 *   CeresInviteCore: 0xe63B92A7873b7cFf52fC5DA1D3dd0440f2Da7178
 *   CeresRegistry:   0x55C1364E46B8Bef987559608e3d831e7D47F1f35
 *   CeresDID:         0x08236d3246653C4699CBBe4458efdC0f5B067250
 */
...
// OxaChain L1 (import from wagmi config)
const oxaChain = config.chains.find(c => c.id === 19505)!;

// ── OxaChain L1 主网 (Chain ID 19505) ──

const TARGET_CHAIN_ID = 19505;

// ── OxaChain L1 主网合约 ──

const INVITE_CORE = '0xe63B92A7873b7cFf52fC5DA1D3dd0440f2Da7178' as const;
const REGISTRY = '0x55C1364E46B8Bef987559608e3d831e7D47F1f35' as const;
const CERES_DID = '0x08236d3246653C4699CBBe4458efdC0f5B067250' as const;
...
export default function CeresMintPage({ myAddress, inviterAddress, pubkeyJson, onDone }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 2. Chain check (tokenOf on OxaChain L1)
  ...
  async function handleMint() {
    ...
    // Must be on OxaChain L1
    if (Number(chainId) !== TARGET_CHAIN_ID) {
      try {
        switchChain?.({ chainId: oxaChain.id as any });
        await new Promise(r => setTimeout(r, 1000));
        if (Number(chainId) !== TARGET_CHAIN_ID) throw new Error('switch chain failed');
      } catch (e) {
        setErrMsg('请切换到 OxaChain 主网（Chain ID 19505）');
        return;
      }
    }
    ...
    try {
      ...
      try {
        const feeEnabled = await readContract(config, {
          ...
          functionName: 'mintFeeEnabled', chainId: oxaChain.id as any,
        } as any);
        if (feeEnabled) {
          const fee = await readContract(config, {
            address: REGISTRY, abi: REGISTRY_ABI,
            functionName: 'mintFee', chainId: oxaChain.id as any,
          } as any);
      ...
      if (pubkeyJson) {
        try {
          ...
          const tokenId = await readContract(config, {
            address: REGISTRY, abi: REGISTRY_ABI,
            functionName: 'tokenOf', args: [account.address], chainId: oxaChain.id as any,
          } as any);
