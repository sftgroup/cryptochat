                ...
                {moments.map((m: any, i: number) => (
                  <div key={m.id || i} className="bg-white border border-gray-200 rounded-xl p-3.5">
                    ...
                    <div className="flex items-center gap-4 pl-10 mt-2 pt-2 border-t border-gray-100">
                      ...
                      {myAddress && (m.authorAddr || m.userId)?.toLowerCase() === myAddress.toLowerCase() && (
                        <button onClick={async () => {
                          if (!confirm('Delete this moment?')) return;
                          try {
                            const r = await fetch(`/api/moments/${m.id}`, { method: 'DELETE', headers: authStore.headers() });
                            if (r.ok) setMoments(prev => prev.filter(x => x.id !== m.id));
                          } catch {}
                        }}
                          className="flex items-center gap-1 text-xs font-medium text-gray-400 hover:text-red-500 transition-colors ml-auto" title="Delete">
                          🗑
                        </button>
                      )}
                    </div>
                    ...
                    <div key={msg.id || i} className={`flex items-end gap-2 ${isSent ? 'flex-row-reverse' : ''}`}>
                      ...
                      <div className={isSent ? 'flex flex-col items-end' : ''} style={{maxWidth:'65%'}}>
                        ...
                        <span className="text-[10px] text-gray-400 mt-0.5 px-1">
                          ...
                          {isSent && activeChat?.type === 'dm' && (
                            <span className="ml-1" title={msg.read ? 'Read' : 'Sent'}>{msg.read ? '✓✓' : '✓'}</span>
                          )}
                        </span>
                      </div>
                    </div>
