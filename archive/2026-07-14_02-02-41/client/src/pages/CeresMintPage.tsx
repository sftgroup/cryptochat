    ...
    try {
      ...
      try {
        const alreadyInvited = await readContract(config, {
          ...
          args: [account.address],
          chainId: TARGET_CHAIN_ID,
          // @ts-ignore -- wagmi v3.7.1 writeContract chainId type mismatch on build
      ...
      try {
        const feeEnabled = await readContract(config, {
          ...
          functionName: 'mintFeeEnabled', chainId: TARGET_CHAIN_ID,
        } as any);
        if (feeEnabled) {
          const fee = await readContract(config, {
            address: REGISTRY, abi: REGISTRY_ABI,
            functionName: 'mintFee', chainId: TARGET_CHAIN_ID,
          } as any);
      ...
      if (pubkeyJson) {
        try {
          ...
          const tokenId = await readContract(config, {
            address: REGISTRY, abi: REGISTRY_ABI,
            functionName: 'tokenOf', args: [account.address], chainId: TARGET_CHAIN_ID,
          // @ts-ignore -- wagmi v3.7.1 writeContract chainId type mismatch on build
